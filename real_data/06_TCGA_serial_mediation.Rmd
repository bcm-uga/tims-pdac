...
# serial mediation with significative AMR from tobacco -> AMR -> survival mediation and immune cells type (V2 and K=8)
```{r serial mediation with LF in covars}
...
saveRDS( LF, "results/06_tcga_med_tobacco_dnam_V2_K8_corrected_LF_matrix.rds")
covars = data.frame(grade = covar[,1], age = covar[,2], gender = covar[,3], lf1 = res_med$hdmax2_step1_param$AS_1$U[,1], lf2 = res_med$hdmax2_step1_param$AS_1$U[,2],lf3 = res_med$hdmax2_step1_param$AS_1$U[,3],lf4 = res_med$hdmax2_step1_param$AS_1$U[,4],lf5 = res_med$hdmax2_step1_param$AS_1$U[,5],lf6 = res_med$hdmax2_step1_param$AS_1$U[,6],lf7 = res_med$hdmax2_step1_param$AS_1$U[,7],lf8 = res_med$hdmax2_step1_param$AS_1$U[,8])

immune_cons$all = rowSums(immune_cons)

mean_meth_AMR = as.matrix(mean_meth_AMR[,1:50])
immune_cons = as.matrix(immune_cons)


res_mediation <- run_serial_mediation_grid2(
  mat_dmr = mean_meth_AMR,
  mat_cell = immune_cons,
  expo = expo,
  covars = covars,
  time = time,
  status = status,
...
colnames(res_mediation) = c("estimate","ci_low","ci_high","med1", "med2", "effect_type")

saveRDS(res_mediation, "results/06_TCGA_serial_med_top50_V2_K8_corrected_with_LF.rds")

```

```{r serial mediation without LF in covars}
...
saveRDS(res_mediation, "results/06_TCGA_serial_med_top50_V2_K8_corrected_without_LF.rds")
...
```{r serial mediation with LF A, B and F in covars}
...
LF = res_med$hdmax2_step1_param$AS_1$U
saveRDS( LF, "results/06_tcga_med_tobacco_dnam_V2_K8_corrected_LF_matrix.rds")
covars = data.frame(grade = covar[,1], age = covar[,2], gender = covar[,3], lf1 = res_med$hdmax2_step1_param$AS_1$U[,1], lf2 = res_med$hdmax2_step1_param$AS_1$U[,2],lf6 = res_med$hdmax2_step1_param$AS_1$U[,6])
...
saveRDS(res_mediation, "results/06_TCGA_serial_med_top50_V2_K8_corrected_with_A_B_F_LF.rds")
...
DAGvalidated_CAT1 <- readRDS("~/Datas/projects/thema_surv/real_data/results/05_DAG_validated_CAT1.rds")
DAGvalidated_CAT1 <- gsub("_", " ", DAGvalidated_CAT1)

DAGvalidated_CAT2 <- readRDS("~/Datas/projects/thema_surv/real_data/results/05_DAG_validated_CAT2.rds")
DAGvalidated_CAT2 <- gsub("_", " ", DAGvalidated_CAT2)

select_LF = readRDS("~/Datas/projects/thema_surv/real_data/results/05_signLFs_by_pairs-A-I.rds")

#saveRDS( LF, "results/06_tcga_med_tobacco_dnam_V2_K8_corrected_LF_matrix.rds")
#covars_LF = data.frame(grade = covar[,1], age = covar[,2], gender = covar[,3], lf1 = res_med$hdmax2_step1_param$AS_1$U[,1], lf2 = res_med$hdmax2_step1_param$AS_1$U[,2],lf6 = res_med$hdmax2_step1_param$AS_1$U[,6])
...
# # AMR14 Bcells
# 
# mean_meth_AMR_temp = mean_meth_AMR$AMR14
# mean_meth_AMR_temp = as.matrix(mean_meth_AMR_temp)
# 
# immune_cons_temp = immune_cons$`B cells`
# immune_cons_temp = as.matrix(immune_cons_temp)
...
#   R = 1000  # pour aller plus vite
# )
# res_mediation_temp$feat = c(rep("AMR46_all",5))
# 
# res = rbind(res, res_mediation_temp)
# 
...
res = readRDS("results/06_TCGA_CAT1_serial_med_V2_K8_corrected_with_selected_LF.rds")

 p = plot_all_pairs_all_effects(res)

ggsave("figures/06_TCGA_pairs_CAT1_with_select_LF.pdf",
       p, width = 14, height = 6, units = "in")
```
...
ggsave("figures/06_TCGA_pairs_CAT2_with_select_LF.pdf",
       p, width = 14, height = 6, units = "in")
...
DAGvalidated_CAT1 <- readRDS("~/Datas/projects/thema_surv/real_data/results/05_DAG_validated_CAT1_withLF.rds")
DAGvalidated_CAT1 <- gsub("_", " ", DAGvalidated_CAT1)

DAGvalidated_CAT2 <- readRDS("~/Datas/projects/thema_surv/real_data/results/05_DAG_validated_CAT2_withLF.rds")
DAGvalidated_CAT2 <- gsub("_", " ", DAGvalidated_CAT2)
...
# colnames(res_mediation) = c("estimate","ci_low","ci_high","med1", "med2", "effect_type")
# 
# saveRDS(res_mediation, "results/06_TCGA_serial_med_top50_V2_K8_corrected_without_LF_V2.rds")
