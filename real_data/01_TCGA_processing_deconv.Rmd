...
Consensus deconvolution method that integrates both bulk and single-cell reference datasets to enhance accuracy and robustness. 

```{r fix_var, include=FALSE}
if (exists("var", inherits = TRUE) && !is.function(var)) rm("var", inherits = TRUE)
var <- stats::var
```

```{r reference_profiles}
...
library(InstaPrism)
library(MuSiC)
ref_rna_scpeng = readRDS("tcga_pdac_mediation/results/test/00_sc_peng.rds")
ref_rna_scragh = readRDS("tcga_pdac_mediation/results/test/00_sc_raghavan.rds")
```
...
# 
# data <- read.table("tcga_pdac_mediation/data/TCGA_PAAD/clinical.project-tcga-paad.2024-07-30/exposure.tsv", 
#                    header = TRUE,       # TRUE si la 1re ligne contient les noms de colonnes
#                           # séparateur tabulation
#                    stringsAsFactors = FALSE)
data_raw <- read.delim("tcga_pdac_mediation/data/TCGA_PAAD/clinical.project-tcga-paad.2025-09-24/exposure.tsv", 
                   header = TRUE, 
                   stringsAsFactors = FALSE)

data = data_raw[,c(1,2,3,36)]

data <- data[ !grepl("--", data$exposures.tobacco_smoking_status %>% trimws()) | is.na(data$exposures.tobacco_smoking_status), ]

# Filter smoking unrelevant categories
raw_exp = data

dim(raw_exp[raw_exp$exposures.tobacco_smoking_status=="Unknown",])

raw_exp = raw_exp[raw_exp$exposures.tobacco_smoking_status!="Unknown",]

dim(raw_exp[raw_exp$exposures.tobacco_smoking_status=="Current Reformed Smoker, Duration Not Specified",])

raw_exp = raw_exp[raw_exp$exposures.tobacco_smoking_status!="Current Reformed Smoker, Duration Not Specified",]

rownames(raw_exp) = paste0(raw_exp$cases.submitter_id, "-01A")



idx_to_keep = meta$Tumor.Sample.ID[which(meta$Tumor.Sample.ID %in% colnames(data_trans$data))]
idx_to_keep = idx_to_keep[idx_to_keep %in% rownames(raw_exp)]
...
idx_na_rows = apply(dat_meth, 1, function(x)
  any(is.na(x)))
dat_meth = dat_meth[!idx_na_rows, idx_to_keep]
dim(dat_meth)
...
dat_meth = dat_meth[probes_chr,idx_to_keep]
...
smoking = raw_exp$exposures.tobacco_smoking_status
smoking_bin = smoking
smoking_bin[smoking=="Lifelong Non-Smoker"] = 0
smoking_bin[smoking=="Current Reformed Smoker for > 15 yrs"] = 0
smoking_bin[smoking=="Current Reformed Smoker for < or = 15 yrs"] = 1
smoking_bin[smoking=="Current Smoker"] = 1
# # alcohol exposure
