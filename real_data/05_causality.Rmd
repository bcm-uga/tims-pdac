...
AMR_matrix = readRDS("~/projects/thema_surv/real_data/results/04_tcga_imm_AMR_matrix_fdr0_05_V2_K8_corrected.rds")
tcga_data = readRDS("~/projects/thema_surv/real_data/results/01_tcga_data_expo_deconv.rds")

# for empirical pvalues : load results from Nelle analysis
amr_to_imm = read.csv("~/projects/thema_surv/real_data/results/amr_to_imm.csv", header = TRUE) 
ANM_CDS = read.csv("~/projects/thema_surv/real_data/results/amr_to_imm_ANM_CDS.csv", header = TRUE) 
...
datAMR = read.csv2("~/projects/thema_surv/real_data/results/03_tcga_AMR_mean_meth_top50_fdr0_05_V2_K8_corrected.csv", header = TRUE, stringsAsFactors = FALSE, row.names = 1, sep = ",") #to check
...
#layout(matrix(1:2, nrow = 1)) #the TRUE model show independance between var and model error
  #  plot(res$x1, res$forward, main = "x1 -> x2")
...
#save results
saveRDS(res_causality, "~/projects/thema_surv/real_data/results/05_tcga_causality.rds")
```
...
res_CAT1
rownames(res_CAT1)
saveRDS(rownames(res_CAT1), file = "~/projects/thema_surv/real_data/results/05_DAG_validated_CAT1.rds")

# Directed acyclic graph 2
res_CAT2 = num_res[  num_res$part_IMM_sign_to_surv < pval_thres &
            num_res$part_AMR_sign_to_surv_when_IMM_controlled >= pval_thres, ]
rownames(res_CAT2)
saveRDS(rownames(res_CAT2), file = "~/projects/thema_surv/real_data/results/05_DAG_validated_CAT2.rds")
...
res_CAT1
rownames(res_CAT1)
saveRDS(rownames(res_CAT1), file = "~/projects/thema_surv/real_data/results/05_DAGvalidated_CAT1.rds")
...
res_CAT2
saveRDS(rownames(res_CAT2), file = "~/projects/thema_surv/real_data/results/05_DAGvalidated_CAT2.rds")
rownames(res_CAT2)
...
![Causal DAG](figures/causal_schema.png)
...
#   #DMR = datAMR[[name]],
#   imm = datIMM[ ,imm]
#   # imm_all = datIMM$Macrophages
# )
# 
# summary(lm(smoking_status~imm+DMR,df_thema))
# 
# summary(lm(smoking_status~imm,df_thema))
# 
# abs(cor(df_thema)) < 0.01
# 
# pc$learn(df_thema)
# pc_mat <- pc$causal_matrix
# pc_g <- hm(pc_mat, "PC")
