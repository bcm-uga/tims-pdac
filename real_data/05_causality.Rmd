...
# Perform a Wilcoxon test between each numeric column and Smoking_Status
...
# Columns corresponding to cell types (everything except Smoking_Status)
cell_vars <- setdiff(names(cur), "Smoking_Status")

# Manually create a long dataframe (without tidyr)
cur_long <- data.frame(
...
p = ggplot(cur_long, aes(x = Smoking_Status, y = Abundance, fill = Smoking_Status)) +
  ...
  geom_boxplot(width = 0.1, outlier.shape = NA) +  # Optional: add boxplot inside
  facet_wrap(~ Cell_Type, scales = "free_y") +
  theme_minimal() +
  labs(title = "Violin plots of cell types by smoking status",
       x = "Smoking status",
       y = "Abundance") +
