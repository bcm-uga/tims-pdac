...
med_tobacco_dnam = readRDS("results/old_hima_version/02_tcga_med_tobacco_dnam_hima.rds")
data_tcga = readRDS("results/01_tcga_data_expo_deconv.rds")
...
# 2) Construction du vecteur CpG → AMR
AMR_list   <- TCGA_top50_AMR$cpg_related_50
AMR_probes <- setNames(
  rep(names(AMR_list), lengths(AMR_list)),  # valeurs = nom de l’AMR
  unlist(AMR_list)                          # noms  = identifiants CpG
)

# 3) Sélection des CpG communs (ici sur colnames, car probes en colonnes)
common_probes <- intersect(colnames(data_icgc$M), names(AMR_probes))
if (length(common_probes) == 0) {
  stop("Aucun CpG commun trouvé : vérifiez colnames(data_icgc$M) et names(AMR_probes)")
}

# 4) Extraction des β-values et calcul des moyennes par AMR
meth_sel       <- data_icgc$M[, common_probes, drop = FALSE]
AMR_labels     <- AMR_probes[common_probes]

# apply sur chaque ligne (= échantillon) pour obtenir un vecteur AMR × samples
AMR_means_icgc <- apply(  meth_sel, 1,
  function(beta_sample) {
    tapply(beta_sample, AMR_labels, mean)
  }
)
# Résultat : matrice n_AMR × n_samples

# 5) Sauvegarde des moyennes
saveRDS(AMR_means_icgc, "results/03_icgc_tcgatop50AMR_meth_mean.rds")

# 6) Analyse de survie (inchangé)
source("surv_parametric_alternative.R")
icgc_res_med_cpg      <- readRDS("results/02_icgc_med_tobacco_dnam_V2_K8_corrected.rds")

# Re-ordonne les échantillons et transpose pour matcher samples×covariables
t_icgc_AMR_means <- t(AMR_means_icgc[, rownames(icgc_res_med_cpg$m_param), drop = FALSE])

hdmax2_step2 <- estimate_effect_surv_param(icgc_res_med_cpg$hdmax2_step1_param,  m = t_icgc_AMR_means)

saveRDS(
  hdmax2_step2,
  "results/03_icgc_med_AMR_estimate_effect.rds"
)
