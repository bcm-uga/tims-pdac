...
#'
#' @export
hima_survival_alter <- function(X, M, OT, status, COV = NULL,
                          topN = NULL,
                          scale = TRUE,
                          FDRcut = 0.05,
                          verbose = FALSE) {
  ...
  pvalues_SIS <- matrix(0, 1, p)## PITTION
  ...
  for (i in 1:p) {
    ...
    pvalues_SIS[i] <- summary(fit)$coefficients[1, 4] # PITTION Extract p-value for each mediator
  }
  ...
  out_result = list(out_result= out_result, pvalue_SIS = pvalues_SIS)#PITTION
