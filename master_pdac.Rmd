...
```{r child="real_data/01_TCGA_processing_deconv.Rmd"}
```
## Step 2 Mediation
...
```{r child="real_data/02_TCGA_mediation.Rmd"}
```
...
```{r child="real_data/03_TCGA_survival_deconv_tobacco_AMR.Rmd"}
```
## Step 4 Correlation
...
```{r child="real_data/05_causality.Rmd"}
```
...
```{r child="real_data/06_TCGA_serial_mediation.Rmd"}
```
