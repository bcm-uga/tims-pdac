...
surv_aalen_med_test = function(object,
  ...
  for (j in 1:p) { 
    if(is.null(covar)){
      ...
      aalen_model = timereg::aalen(base ~ M[,j] + exposure + object$U)
      pvalues = c(pvalues, aalen_model$pval.testBeq0[2])
    } else {
      aalen_model = timereg::aalen(base ~ M[,j] + exposure + object$U + covar)
      pvalues = c(pvalues, aalen_model$pval.testBeq0[2])
    }
  }
  res = list(pvalues = pvalues)
    ...
    if (expo_var_type == "character"||is.factor(exposure)){
      ...
      for (k in 1:length(cn)) {
        ...
        for (i in 1:ncol(M)) {#numeric
          ...
          model.y = mod2
          #########################################
          # avoid rmvrnorm error
          # Check if the required package is available
          if (!requireNamespace("mvtnorm", quietly = TRUE)) {
            stop("The 'mvtnorm' package is required for multivariate normal simulations. Please install it.")
          }
          
          # Extract and verify gamma and robust variance-covariance matrix
          if (!is.null(model.y$gamma) && !is.null(model.y$robvar.gamma)) {
            gamma <- model.y$gamma
            covmat <- model.y$robvar.gamma
          } else if (!is.null(summary(model.y)$gamma)) {
            gamma <- summary(model.y)$gamma
            covmat <- summary(model.y)$robvar.gamma
          } else {
            stop("Could not find 'gamma' and 'robvar.gamma' in the 'model.y' object.")
          }
          
          # Ensure the covariance matrix is in matrix format
          covmat <- as.matrix(covmat)
          
          # Fix symmetry issues if necessary
          if (!isSymmetric(covmat)) {
            warning("The robust variance-covariance matrix is not symmetric. It will be symmetrized.")
            covmat <- 0.5 * (covmat + t(covmat))
          }
          
          # Check if the covariance matrix is positive semi-definite
          eigenvalues <- eigen(covmat, symmetric = TRUE)$values
          if (any(eigenvalues < -1e-6)) {
            stop("The robust covariance matrix is not positive semi-definite.")
          }
          
          # Example usage (optional)
          # samples <- mvtnorm::rmvnorm(1000, mean = gamma, sigma = covmat)
          ######################################################################
          
          med = multimediate::multimediate(lmodel.m = mod1, correlated=FALSE, model.y, treat = "exposure")
