...
run_AS_surv_param = function(exposure,
  ...
  if(expo_var_n == 1){
    ...
    env = as.matrix(cbind(exposure, covar))
    ...
    if (expo_var_types == "character"|| is.factor(exposure_input)){
      ...
      env = as.matrix(cbind(exposures, covar))
...
estimate_effect_surv_param <- function(object , m, boots = 1000, ...) {
  ...
  survival_time = as.numeric(object$input$survival_time_input)
  ...
  for(expo_var_id in expo_var_ids){
    ...
    if (expo_var_type == "character"||is.factor(exposure)){
      ...
      for (k in 1:length(cn)) {
        ...
        tmp = list(ACME=ACME, ADE=ADE, PM=PM, TE=TE, xm=xm, my=my)
      ...
      tmp = list(ACME=ACME, ADE=ADE, PM=PM, TE=TE, xm=xm, my=my, oie=oie, oie_med=oie_med, oie_sd=oie_sd, ote=ote, ode=ode, best_distribution= best_distribution)
